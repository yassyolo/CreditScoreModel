...
def feature_engineering(data):
    ...
    # Polynomial features for selected interactions
...
enn = NearestNeighbors(n_neighbors=5)  # Adjust n_neighbors as needed
enn.fit(X_resampled)  # Use X_resampled here

distances, indices = enn.kneighbors(X_resampled)
neighbors_avg = distances.mean(axis=1)
threshold = np.percentile(neighbors_avg, 75)  # You can adjust the threshold
...
# Combine SMOTE and ENN results (optional)
...
# Define and tune Gradient Boosting model
gb_model = GradientBoostingClassifier(random_state=42)

# Expanded RandomizedSearchCV parameters
param_dist = {
...
def calculate_credit_score(probability, min_score=300, max_score=850):
    """
    Calculate the credit score based on the probability of loan approval.

    :param probability: Probability of loan approval (from the model).
    :param min_score: Minimum credit score in the range.
    :param max_score: Maximum credit score in the range.
    :return: Credit score based on the probability.
    """
    # Ensure the probability is within the range [0, 1]
    probability = np.clip(probability, 0, 1)

    # Linearly map probability to the credit score range
...
def assess_risk(probability, credit_score, min_score=300, max_score=850):
    """
    Assess the risk based on the probability of loan approval and credit score.

    :param probability: Probability of loan approval (from the model).
    :param credit_score: Credit score calculated from the probability.
    :param min_score: Minimum credit score in the range.
    :param max_score: Maximum credit score in the range.
...
# Apply feature engineering to example data
